...
public class Topology : ITopology
{
    ...
    public async Task UnbindDispatch<TMessage, TResult>(string routingKey = "")
    {
        await _dispatchPipeBinder.Unbind<TMessage, TResult>(_dispatchPipe, routingKey);
    }
    ...
    {
        await _receivePipeBinder.Bind<TMessage, TResult>(_receivePipe);
        _handlerBinder.Bind<TMessage, TResult, THandler>(routingKey);
    }
    ...
    public async Task UnbindReceive<TMessage, TResult>(IHandler<TMessage, TResult> handler, string routingKey = "")
    {
        await _receivePipeBinder.Unbind<TMessage, TResult>(_receivePipe);
        _handlerBinder.Unbind(handler, routingKey);
    }
    ...
    public async Task UnbindReceive<TMessage, TResult, THandler>(string routingKey = "")
        where THandler : IHandler<TMessage, TResult>
    {
        await _receivePipeBinder.Unbind<TMessage, TResult>(_receivePipe);
        _handlerBinder.Unbind<TMessage, TResult, THandler>(routingKey);
    }
}